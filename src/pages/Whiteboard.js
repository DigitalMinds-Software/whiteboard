...
const Whiteboard = () => {
  ...
  const { id } = useParams();
  const { setCurrentWhiteboard, whiteboards, updateWhiteboardElement } = useContext(WhiteboardContext);
  ...
  const handleElementUpdate = useCallback((elementId, newData) => {
    updateWhiteboardElement(id, elementId, newData);
  }, [id, updateWhiteboardElement]);
  ...
  const handleAddElement = useCallback((newElement) => {
    ...
    updateDocument(`whiteboards/${id}/elements`, elementToAdd);
  }, [id, user.uid]);
